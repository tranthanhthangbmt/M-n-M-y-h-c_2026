# Kế Hoạch Tạo Script Demo - Chương 12 (Trí Tuệ Nhân Tạo: Giới Thiệu)

## 1. Mục Tiêu
Tạo một kịch bản (script) chi tiết phục vụ cho việc tạo video/audio bài giảng Chương 12 - Môn Máy học. Script sẽ mô phỏng một buổi học trên lớp bậc đại học, dưới dạng cuộc hội thoại trao đổi hai chiều giữa Giảng viên (Người 1) và Sinh viên (Người 2), nhằm giúp sinh viên hiểu sâu lý thuyết, phương pháp và ví dụ thực tiễn.

## 2. Dữ Liệu Đầu Vào
- **File Slide PDF:** `slideML/Slide_ML_Chap12.pdf` (Chứa các slide trình chiếu của Chương 12).
- **Nội Dung Chi Tiết:** `machineLearningWeb/docs/chuong_12.md` (Chứa phần lý thuyết nền tảng, công thức và giải thích chi tiết cho từng phần của chương).

## 3. Cấu Trúc Thư Mục & File Đầu Ra
- **Kiểm tra và tạo thư mục:** Nếu thư mục `machineLearningWeb/Video/Chapter_12` chưa tồn tại, hệ thống (hoặc người thực hiện) sẽ tạo mới thư mục này.
- **Copy File:** Copy file `Slide_ML_Chap12.pdf` từ thư mục `slideML` vào thư mục `machineLearningWeb/Video/Chapter_12`.
- **File Script:** Tạo và lưu file script (ví dụ: `scriptCh12.txt` hoặc cập nhật `scriptDemo.txt`) trong thư mục `machineLearningWeb/Video/Chapter_12`.
...
2. **Tiến trình bài giảng:** 
   ...
   - Nội dung thảo luận trên mỗi slide được trích xuất và tham chiếu sâu từ file `chuong_12.md` để đảm bảo độ chi tiết như một tiết học thực tế ở bậc đại học.
3. **Mức độ chi tiết:** 
...
- **Bước 1 (Chuẩn bị không gian làm việc):** Khởi tạo thư mục `Chapter_12` trong `Video`, copy slide PDF sang thư mục mới.
- **Bước 2 (Phân tích dữ liệu):** Đọc nội dung Markdown `chuong_12.md` và đối chiếu với cấu trúc `Slide_ML_Chap12.pdf`. Mapping nội dung từng phần trong docs vào từng slide tương ứng.
...
- **Bước 4 (Soạn thảo kịch bản - Thân bài):** Triển khai viết script cho từng slide. Tại mỗi slide:
  ...
  - Thầy giáo giải thích chi tiết, đọc rõ các công thức bằng chữ, đưa ra ví dụ (dựa trên `chuong_12.md`).
...
- Thư mục `machineLearningWeb/Video/Chapter_12` đã có đủ slide và file script.
