...
html_path = r'd:\DongAUniversity\TÀI LIỆU DẠY HỌC_2024-2025\Môn Máy học_2026\Tài liệu\CHƯƠNG 10.htm'
ch10_dir = r'd:\DongAUniversity\TÀI LIỆU DẠY HỌC_2024-2025\Môn Máy học_2026\machineLearningWeb\Figures\CH10'
    ...
    for _ in range(5):
        if curr:
            ...
            if curr:
                ...
                if re.search(r'Hình\s*10-\d+', text, re.IGNORECASE):
                    caption_text = text
                    break
    
    if caption_text:
        match = re.search(r'(Hình\s*10-\d+)', caption_text, re.IGNORECASE)
        if match:
...
for hinh, imgs in hinh_to_images.items():
    ...
    best_img = None
    for img in imgs:
        path = os.path.join(ch10_dir, img)
...
for img, hinh in final_mapping.items():
    ...
    # Rename the file in CH10
    src_path = os.path.join(ch10_dir, img)
    ext = os.path.splitext(img)[1]
    dst_path = os.path.join(ch10_dir, hinh + ext)
