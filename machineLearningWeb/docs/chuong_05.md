...
Để giải quyết trọn vẹn và sâu sắc **Phần 2 (từ Bài 9 đến Bài 11)** của Chương 5, chúng ta cần phân tích kỹ lưỡng các yêu cầu thực hành nâng cao liên quan đến **Máy vectơ hỗ trợ (SVM)** [cite: 840, 871]. Các bài tập này chuyển dịch từ việc hiểu sâu bản chất các lớp ước lượng trong Scikit-Learn sang xây dựng hệ thống phân loại đa lớp thực tế và giải quyết bài toán hồi quy phi tuyến quy mô lớn [cite: 44, 45, 47]:

1.  **Bài 9 (Đồng bộ hóa mô hình tuyến tính)**: Yêu cầu huấn luyện `LinearSVC`, `SVC(kernel="linear")`, và `SGDClassifier` trên tập dữ liệu Iris phân tách tuyến tính sao cho kết quả trả về gần như trùng khớp hoàn toàn [cite: 44]. Ta cần phân tích công thức toán học để đồng bộ hóa mức độ điều chuẩn (Regularization) thông qua mối liên hệ giữa siêu tham số \\(C\\) của SVM và hệ số phạt \\(\alpha\\) của SGD [cite: 855].
2.  **Bài 10 (Phân loại đa lớp thực tế trên tập Wine)**: Sử dụng các thuộc tính hóa học của rượu vang để dự đoán nhà sản xuất [cite: 45]. Đây là bài toán đa lớp [cite: 45]. Chúng ta cần thảo luận tầm quan trọng của việc chuẩn hóa dữ liệu (Z-score Scaling) [cite: 98, 870], chiến lược One-vs-Rest (OvR) [cite: 45], và áp dụng `GridSearchCV` để tối ưu hóa mô hình phi tuyến [cite: 690].
3.  **Bài 11 (Hồi quy SVM quy mô lớn)**: Dự đoán giá nhà California bằng `SVR` [cite: 47, 706]. Do kích thước tập dữ liệu lớn (>20,000 mẫu), việc huấn luyện SVR phi tuyến trên toàn bộ tập dữ liệu sẽ cực kỳ chậm (độ phức tạp \\(O(m^2)\\) đến \\(O(m^3)\\)) [cite: 45, 47, 855]. Ta cần áp dụng chiến lược tinh chỉnh siêu tham số nhanh bằng **tập con 2,000 mẫu** thông qua `RandomizedSearchCV`, sau đó mới huấn luyện mô hình tốt nhất trên toàn bộ dữ liệu [cite: 13, 16, 47].

---

### PHẦN 2: Chi tiết bài tập và lời giải thực hành (Bài 9 đến Bài 11)
