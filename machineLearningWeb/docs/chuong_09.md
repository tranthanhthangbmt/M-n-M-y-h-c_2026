...
Để giải quyết trọn vẹn và tối ưu nhất nhóm bài tập thực hành cuối cùng của **Chương 9: Các kỹ thuật học không giám sát** (Bài 10, 11, 12) liên quan đến tập dữ liệu khuôn mặt Olivetti cổ điển, chúng ta sẽ áp dụng phương pháp lập luận từng bước (Chain of Thought). Quy trình phân tích dữ liệu, thiết kế hệ thống và giải thuật lập trình được mô tả chi tiết như sau:

1.  **Phân tích đặc trưng của tập dữ liệu khuôn mặt Olivetti**:
...
2.  **Lập luận giải thuật cho Bài 10 (Phân cụm K-Means & Bộ phân loại)**:
    ...
    *   *Sử dụng K-Means làm công cụ giảm chiều*: Khoảng cách từ một bức ảnh tới \\(k\\) tâm cụm (tính qua `kmeans.transform()`) tạo ra một không gian đặc trưng mới gồm \\(k\\) chiều [cite: 627, 637]. Ta sẽ kiểm chứng hiệu suất phân loại của mô hình (ví dụ: `RandomForestClassifier`) trên 3 không gian: ảnh gốc (hoặc PCA), chỉ dùng khoảng cách K-Means, và gộp cả hai không gian [cite: 650].

3.  **Lập luận giải thuật cho Bài 11 (Mô hình hỗn hợp Gaussian GMM & Tạo ảnh mới & Phát hiện dị thường)**:
    *   *Tại sao chọn GMM trên không gian PCA?* GMM có độ phức tạp tính toán rất lớn theo số lượng chiều dữ liệu vì phải ước tính ma trận hiệp phương sai [cite: 645, 651]. Chạy GMM trực tiếp trên 4096 chiều sẽ gây sập bộ nhớ và không thể hội tụ. Do đó ta phải huấn luyện GMM trên không gian PCA (~199 chiều) [cite: 651].
    *   *Tạo ảnh khuôn mặt mới*: GMM là một mô hình xác suất sinh [cite: 513]. Sử dụng `gmm.sample()` để sinh ra các vector đặc trưng mới trong không gian tiềm ẩn của PCA [cite: 651], sau đó dùng `pca.inverse_transform()` để chiếu ngược lại không gian ảnh \\(64 \times 64\\) để hiển thị [cite: 651].
    *   *Phát hiện dị thường*: Ta chủ động tạo ra các dị thường bằng cách xoay ảnh, lật ngược ảnh, hoặc làm tối ảnh [cite: 651]. Sử dụng `gmm.score_samples()` để tính log-likelihood (mật độ xác suất) [cite: 625, 651]. Các ảnh dị thường sẽ có điểm số mật độ xác suất cực kỳ thấp so với ảnh bình thường [cite: 625, 651].

4.  **Lập luận giải thuật cho Bài 12 (Phát hiện dị thường bằng sai số tái tạo PCA)**:
    *   *Nguyên lý*: PCA học không gian con tuyến tính đại diện cho các khuôn mặt bình thường, thẳng đứng [cite: 652]. Khi đưa một ảnh dị thường (ví dụ: bị xoay ngược \\(180^\circ\\) hoặc bôi đen) qua đường ống `pca.transform()` rồi `pca.inverse_transform()` để tái tạo [cite: 652], mô hình PCA sẽ cố gắng tái tạo nó thành một khuôn mặt bình thường thẳng đứng [cite: 652]. Điều này tạo ra một **sai số tái tạo (reconstruction error)** cực kỳ lớn giữa ảnh gốc và ảnh tái tạo [cite: 647, 652]. Đây là một trong những kỹ thuật phát hiện dị thường đơn giản nhưng cực kỳ hiệu quả [cite: 647].

---

### PHẦN 2: Lời giải chi tiết và mã nguồn Python tối ưu (Bài 10, 11 và 12)
