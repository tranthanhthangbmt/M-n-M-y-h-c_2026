...
\begin{itemize}
    ...
    \item \textbf{Hạ Gradient ngẫu nhiên (SGD)} ngược lại, chọn ra một trường hợp ngẫu nhiên trong tập huấn luyện tại mỗi bước và chỉ tính gradient trên trường hợp duy nhất đó.
...
\begin{itemize}
    \item Tại mỗi bước, thay vì tính toán gradient dựa trên toàn bộ dữ liệu (như Batch GD) hoặc chỉ một trường hợp (như SGD), thuật toán tính gradient trên các tập con nhỏ ngẫu nhiên gọi là \textbf{mini-batch}.
...
\begin{itemize}
    ...
    \item Do đó, Lasso tự động thực hiện \textbf{lựa chọn đặc trưng (feature selection)}, tạo ra một mô hình thưa (sparse model).
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item Mở rộng khả năng của mô hình cho dữ liệu phức tạp bằng \textbf{Hồi quy đa thức}.
    \item Biết cách sử dụng \textbf{Đường cong học tập} để chẩn đoán Underfitting/Overfitting.
    \item Triển khai các phương pháp \textbf{Chính quy hóa} (Ridge, Lasso, Elastic Net, Early Stopping) để ngăn chặn Overfitting.
    \item Làm quen với các công cụ Phân loại xác suất thông qua \textbf{Hồi quy Logistic} và \textbf{Hồi quy Softmax}.
\end{itemize}
\end{frame}
